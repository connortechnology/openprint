...
{
   $result = db_query ("SELECT * FROM domain_admins WHERE username='$username' AND domain='$domain' AND active=true");
   if ($result['rows'] != 1)
   {
      return false;
   }
...
function list_domains_for_admin ($username)
{
   ...
   $result = db_query ("SELECT * FROM domain LEFT JOIN domain_admins ON domain.domain=domain_admins.domain WHERE domain_admins.username='$username' AND domain.active=true AND domain.backupmx=false ORDER BY domain_admins.domain");
   if ($result['rows'] > 0)
   {
      $i = 0;
