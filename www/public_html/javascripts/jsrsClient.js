...
function contextCreateContainer( containerName ){
	...
	switch( jsrsBrowser ) {
		...
		case 'IE':
			...
			var html = '<iframe name="' + containerName + '" src="/blank.asp"></iframe>';
			span.innerHTML = html;
...
function contextPOST( rsPage, func, parms ){
	...
	content += '<form name="jsrsForm" method="post" target="" ';
	content += ' action="' + rsPage + '?U=' + unique + '">';
	content += '<input type="hidden" name="C" value="' + this.id + '">';

	// func and parms are optional
	if (func != null){
		content += '<input type="hidden" name="F" value="' + func + '" />';
		if (parms != null){
			if (typeof(parms) == 'string'){
				// single parameter
				content += '<input type="hidden" name="P0" ' + 'value="[' + jsrsEscapeQQ(parms) + ']">';
			} else {
				// assume parms is array of strings
				for( var i=0; i < parms.length; i++ ){
					content += '<input type="hidden" name="P' + i + '" ' + 'value="[' + jsrsEscapeQQ(parms[i]) + ']">';
				} // end for
			} // parm type
		} // parms
	} // func
//alert(content);
	doc.open();
	doc.write( content + '</form></body></html>');
	doc.close();
	if ( doc.forms['jsrsForm'] )
...
function contextGetPayload(){
	switch( jsrsBrowser ) {
		...
		case 'IE':
		if ( ! this.container.document.forms['jsrs_Form']['jsrs_Payload'] ) {
...
function jsrsExecute( rspage, callback, func, parms, visibility ){
	...
	if ( jsrsPOST && ((jsrsBrowser == 'IE') || (jsrsBrowser == 'MOZ'))){
		if ( ! contextObj.POST( rspage, func, parms ) ) {
			return false;
		} // end if
	} else {
...
function jsrsEscapeQQ( thing ){
	if ( typeof(thing) == 'string' ) {
		...
		//alert("jsrsEscapeQQ: " + thing + typeof(thing));
		return thing;
	} // end if
} // end function jsrsEscapeQQ( thing )
