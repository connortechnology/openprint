...
function calc(){
  ...
  mts = document.autoSumForm.nofmonths1.options[document.autoSumForm.nofmonths1.selectedIndex].value;
  ...
  Singolo1 = ((((qty / tft) * (pap * 1)) + ((qty / tft) * (hol * 1)) + (trn * 1) + (mts * 1) + (clr * 1) + (bpz * 1)) / (qty * 1)); 
  
  stotale1 = (((qty / tft) * (pap * 1)) + ((qty / tft) * (hol * 1)) + (trn * 1) + (mts * 1) + (clr * 1) + (bpz * 1)); 
...
//-->
<!--
function setForm16Value() {
    var selectedItem      = document.autoSumForm.nofmonths1.selectedIndex;
    var selectedItemText  = document.autoSumForm.nofmonths1.options[selectedItem].text;
    
    if (selectedItem != -1) {
        document.autoSumForm.nofmonths.value = selectedItemText;
    }
    else {
        document.autoSumForm.nofmonths.value = "";
