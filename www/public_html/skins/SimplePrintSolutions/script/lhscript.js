...
function calc(){
  ...
  seh = document.autoSumForm.secondsheets1.options[document.autoSumForm.secondsheets1.selectedIndex].value;
  ...
  Singolo1 = ((((qty / tft) * (pap * 1)) + ((qty / tft) * (clr * 1)) + ((qty / tft) * (seh * 1)) + (trn * 1) + (bpz * 1)) / (qty * 1)); 
  
  stotale1 = (((qty / tft) * (pap * 1)) + ((qty / tft) * (clr * 1)) + ((qty / tft) * (seh * 1)) + (trn * 1) + (bpz * 1)); 
...
function setForm8Value() {
    var selectedItem      = document.autoSumForm.secondsheets1.selectedIndex;
    var selectedItemText  = document.autoSumForm.secondsheets1.options[selectedItem].text;
    
    if (selectedItem != -1) {
        document.autoSumForm.secondsheets.value = selectedItemText;
    }
    else {
        document.autoSumForm.secondsheets.value = "";
