...
function calc(){
  ...
  fld = document.autoSumForm.folding1.options[document.autoSumForm.folding1.selectedIndex].value;
  ...
  Singolo1 = ((((qty / tft) * (pap * 1)) + ((qty / tft) * (fld * 1) + (fld * 1)) + (trn * 1) + (clr * 1) + (bpz * 1)) / (qty * 1)); 
  
  stotale1 = (((qty / tft) * (pap * 1)) + ((qty / tft) * (fld * 1) + (fld * 1)) + (trn * 1) + (clr * 1) + (bpz * 1)); 
...
//-->
<!--
function setForm7Value() {
    var selectedItem      = document.autoSumForm.folding1.selectedIndex;
    var selectedItemText  = document.autoSumForm.folding1.options[selectedItem].text;
    
    if (selectedItem != -1) {
        document.autoSumForm.folding.value = selectedItemText;
    }
    else {
        document.autoSumForm.folding.value = "";
