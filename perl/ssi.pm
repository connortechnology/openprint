...
sub write_override {
	...
	if ( 1 ) {
		return sprintf(q`<input type="hidden" id="%1$s" name="%1$s" value="%2$s"/><img class="Override" src="/images/%3$s.gif" alt="" onclick="var e=$('%1$s');if(e.value){e.value='';this.src='/images/unlocked.gif';%5$s} else {e.value='Y';this.src='/images/locked.gif';%4$s}"/>`, $for, $value, ($value ? 'locked' : 'unlocked'), $locked_js, $unlocked_js );
	} else {
