...
sub additem # {{{
{
	...
	push (@{$self->{'items'}}, $item{'quantity'} || 1,
...

{
	...
	unless ($resultcode == 1)
	{
		...
		return $self->_error(undef,$resultmessage);
	}
...

{
	...
	if ($new)
	{
		push (@englishmessages, $new);
		push (@frenchmessages, $new);
