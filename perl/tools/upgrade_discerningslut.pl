...
$src_db = 'discerningslut' if ! $src_db;
$dst_db = 'discerningslut' if ! $dst_db;
$src_host = 'www.discerningslut.com' if ! $src_host;
...
print "upgrading db ...";
`/etc/apache2/lib/perl/tools/db_update3.pl $dst_db discerningslut discerningslut` or $log->error($!);
print 'Turning off backups...';
$dbh = sql::open_sql( $log, ('database'=>$dst_db, 'driver'=>'Pg','login'=>'discerningslut', 'password'=>'discerningslut') );
