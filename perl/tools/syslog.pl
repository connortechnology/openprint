...
my @re = (
		...
		'^(\w{3} [ :0-9]{11}) [\._a-zA-Z0-9\-]+ dovecot: (imap|pop3)\-login: Disconnected (Too many bad commands )?\((no auth|auth failed, [0-9]+) attempts( in [0-9]+ secs)?\): user=<[^>]*>, (method=PLAIN, )?rip=(?<IP>[\.0-9]+), lip=[\.0-9]+(, session=<[^>]+>)?$',
