...
my @whitelist = (
        ...
        q`^\[\w{3} \w{3} [ .:0-9]+\] \[mpm_prefork:notice\] \[pid [0-9]+\] AH[[:digit:]]{5}: Apache\/[.[:digit:]]+ \(Ubuntu\) (mod_fcgid\/2\.3\.9 )?( PHP\/[.[:alnum:]-]+ )?(OpenSSL\/[.[:alnum:]]+ )?mod_apreq2\-20090110\/2\.8\.0 mod_perl\/[\.[:digit:]]+ Perl\/v[\.[:digit:]]+ configured \-\- resuming normal operations$`,
