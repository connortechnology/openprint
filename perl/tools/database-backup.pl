...
foreach my $db ( @dbs ) {
	...
	if ( $$row{'backup'} ) {
		...
		if ( $$opts{host} and $$opts{host} ne 'local' ) {
			system("pg_dump -b -h $$opts{host} $db | bzip2 > $$opts{path}/$$opts{host}/$db/$year-$mon-$mday.sql.new.bz2");
		} else {
			system("pg_dump -b $db | bzip2 > $$opts{path}/$$opts{host}/$db/$year-$mon-$mday.sql.new.bz2");
		} # end if
