...
sub to_string {
	if ( ! $_[0]{'to_string'} ) {
		$_[0]{'to_string'} = sprintf('%s%s %dx%d+%dx%d=%dout %s %dx%d=%dpages on %sx%s%s->%sx%s %s', ($_[0]{quantity} > 1 ? $_[0]{quantity} .'x ': '' ),( $_[0]{Press} ? $_[0]->Press()->strid() : 'unknown equipment' ), $_[0]->get('columns','rows','dutch_columns','dutch_rows','imposition','runstyle','page_columns','page_rows','pages', 'paper_width','paper_height', 'paper_type','sheet_width','sheet_height', 'image_orientation') );
	}
