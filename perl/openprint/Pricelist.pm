...
sub find {
	...
	if ( $params{'id'} ) {
		...
		my @values;
		my $sql = q{SELECT id FROM Pricelists WHERE 1>0};
		if ( $params{'name'} ) {
			$sql .= q{ AND name =?};
...
sub load {
	my $self = shift;
	@$self{'id','name','Description', 'currency_id'} = sql::execute( $openprint::log, $openprint::dbh, q{SELECT id, name, Description, currency_id FROM Pricelists WHERE id=?}, $$self{'id'} );
} # end sub load
