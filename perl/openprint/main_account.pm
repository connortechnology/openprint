...
sub select_company {
	if ( $param{'ddmCompany'} ) {
		...
		foreach my $k ( keys %session ) {
			next if sets::isin( $k, [ 'Currency_id', '_session_id','user_id','company_id','user_type','Country','lastupdated' ] );
			delete $session{$k};
		} # end foreach
	} # end if
...
sub registration {
	...
	if ( $config{'UseCaptchaOnRegistration'} eq 'Y' ) {
        ...
        my $Captcha = new Authen::Captcha('data_folder' => '/tmp', 'output_folder' => $ENV{'DOCUMENT_ROOT'}.'/skins/'.$config{'SiteTitle'}.'/images/captcha');
	...
	my ( $cust_id ) = sql::execute( $log, $dbh, q{SELECT Index FROM Company WHERE lower(strName) = lower(?) AND upper(strPostalCode) = ?}, @param{'name','postalcode'} );
	if ( ! $cust_id ) {
...
# We assume that we are authorized to be here now.
	...
	if ( ( $Me->administrator() eq 'Y' ) or ( new openprint::Company( $session{'company_id'} )->salesrep_id() == $Me->id() ) ) {
...
sub logout {
	...
	delete @session{'user_id','company_id','email','user_type','OrderID','project_id','quote_id','Pricelist_id'};
	#openprint::order::delete_unfinished_orders( $openprint::log, $openprint::dbh, $session{_session_id} );
