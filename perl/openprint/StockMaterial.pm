...
sub load {
	...
	if ( (! $data) and $$self{'id'} ) {
		$data = $openprint::dbh->selectrow_hashref( 'SELECT * FROM StockMaterials WHERE id=?', {}, $$self{'id'} );
...
sub delete {
	my $self = shift;
    sql::execute( undef, undef, q{DELETE FROM StockMaterials WHERE id=?}, $$self{'id'} );
} # end sub delete
...
sub save {
	...
	if ( ! $$self{'id'} ) {
		@$self{'id'} = sql::execute( undef, undef, q{SELECT nextval('stockmaterials_id_seq')});
		$sql{'id'} = $$self{'id'};
		if ( my $error = sql::insert( undef, undef, 'StockMaterials', \%sql ) ) {
			sql::end_transaction( $openprint::dbh, $ac );
			return $error;
		} # end if
	} else {
		if ( my $error = sql::update( undef, undef, 'StockMaterials', ['id=?', $$self{'id'}], \%sql ) ) {
			sql::end_transaction( $openprint::dbh, $ac );
			return $error;
		} # end if
	} # end if
...
sub copy {
	my $self = shift;
	my $new = new openprint::StockMaterial();
