...
my @campaign_ids = openprint::EmailCampaign::find( 'active' => 'Y', 'misc' => '(lastrun+interval<now() OR lastrun IS NULL OR nextrun < now()) AND ( timeofday IS NULL or timeofday <= NOW()::time)' );
