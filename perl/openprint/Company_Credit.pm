...
sub warn_orders {
    ...
    AND ( curTotalSale > (SELECT SUM(curAmount) FROM Payments WHERE deleted=false AND completed=true AND Payments.order_id=Orders.Index)
    OR (SELECT SUM(curAmount) FROM Payments WHERE deleted=false AND completed=true AND Payments.order_id=Orders.Index) IS NULL )
