...
sub calc_price {
	...
	$MakeReadyService = openprint::Service->find_one( name => 'Embossing-'.$$specs{'rbEmbossing-'.$form}.'MakeReady' ) if (!$MakeReadyService) and $$specs{'rdbEmbossing-'.$form};
...
# check for a standard die.
		...
		if ( ( ! %DiePrice ) and $$specs{'rdbEmbossing-'.$form} ) {
			if ( my $Material = openprint::Material->find_one( name=>$$specs{'rdbEmbossing-'.$form}.'Die') ) {
				%DiePrice = $Material->get_price( undef, $Equipment );
			} # end if
		} # end if
...
#$log->debug(" ** Adding Bending Cost: $bending_price For $$specs{txtDieCutBends} Bends, MakeReady Total: $make_ready ** ");
			if ( my $Material = openprint::Material->find_one('name'=>'EmbossingDieRule') ) {
...
##punches are optional
				if ( my $Material = openprint::Material->find_one('name'=>'DieCutPunch'.$$specs{'rdbEmbossing-'.$form}) ) {
...
# * $$Imposition{imposition});
	...
	if ( my $Overs = $Equipment->Specification('Embossing Overs') ) {
		my $overs;
...
# this is the price for actual die cutting, priced by impressions.
	my %ServicePrice = openprint::service::get_price_object( 'Embossing'.$$specs{'rdbEmbossing-'.$form}, $impressions, $Equipment );
	if ( ! %ServicePrice ) {
		%ServicePrice = openprint::service::get_price_object( 'Embossing', $impressions, $Equipment );
	} # end if
...
# now we have to make a custom die
		if ( $$specs{'rdbEmbossing-'.$form} eq 'Simple' ) {
			$$specs{'txtSteelRuleLength-'.$form} = 6;
		} elsif ( $$specs{'rdbEmbossing-'.$form} eq 'Average' ) {
			$$specs{'txtSteelRuleLength-'.$form} = 9;
		} elsif ( $$specs{'rdbEmbossing-'.$form} eq 'Complex' ) {
...
#$log->debug("Diecutting::signatureNeeds: for sig $form : Needed: ($$specs{'Needed-'.$form})" );
	if ( ! $$specs{"Needed-$form"} ) {
		...
		if ( $ServiceType ) {
			...
			$openprint::log->error("No ServiceType for Embossing");
		}
	} # end if
...
sub display {
	...
	if ( $$variable{rdbTemplateTypePresentationFolderStandard1Pocket} ne '' or $$variable{rdbTemplateTypePresentationFolderStandard2Pocket} ne '' ) {
		$$variable{ShowPresentationFolderEmbossing} = 'Y';
	} # end if
