...
#$openprint::log->debug("Templatetype: $$sig_specs{'rdbTemplateType'}") if $debug;
						my $rc = $Equipment->fits( $Imposition->sheet_width(), $Imposition->sheet_height() );
...
# See if it fits
						$_ = $Equipment->fits( $Imposition->sheet_width(), $Imposition->sheet_height() );
...
#$openprint::log->debug("Overs: " . $Fold->makeready_overs() );
				...
				$Breakdown .= sprintf( '%s: %dx%dout layout: %sx%s qty: %d<br/>', $Fold->name(), $impo_qty, $imposition, $Imposition->get('sheet_width', 'sheet_height'), $run_qty );
