...
sub impositions {
	...
	$Fold = $Imposition->Equipment()->Fold(%find);
	return @imps if ! $Fold;
...
# See if it fits
			...
			if ( $$specs{"chkOverrideFold-$$sig_specs{'SignatureIndex'}-$qty_index"} eq 'Y' ) {
				...
				my %found;
                foreach my $key ( keys %folds ) {
                    $openprint::log->debug("DUmp folds $key...") if DEBUG;
                }
...
$openprint::log->debug("Folds: $set_index : $key " . $impo_qty );
			...
			foreach my $key ( keys %folds ) {
				...
				if ( $$sig_specs{'txtFinalWidth'} ) {
					$width_folds = Math::Round::nearest(1, ($$sig_specs{'txtWidth'}/$$sig_specs{'txtFinalWidth'})-1 );
					$height_folds = Math::Round::nearest(1, ($$sig_specs{'txtHeight'}/$$sig_specs{'txtFinalHeight'}) -1 );
				} else {
					$width_folds = Math::Round::nearest(1, ($Imposition->image_width()/$Imposition->object_width())-1 );
					$height_folds = Math::Round::nearest(1, ($Imposition->image_height()/$Imposition->object_height())-1 );
				} # end if
				...
				$Breakdown .= sprintf( '%s: %dx%dout layout: %sx%s qty: %d StockWeight %.2fgsm<br/>', $Fold->name(), $impo_qty, $imposition, $Imposition->get('layout_width', 'layout_height'), $run_qty, $Paper->gsm() );
...
# This would stop pricing once it's too expensive... but then we wouldn't get the breakdown 
			...
			my %cutting_results;
			if ( $$services{'Cutting'} and @{$$services{'Cutting'}} ) {
				%cutting_results = openprint::Estimating::Cutting::signature_calc_folding_cutting( $Project, $$services{'Cutting'}[0], $sig_specs, $$calc_hash{cutting_specs}, $qty_index, $Paper, $SignatureImposition, \%fold_specs );
			} # end if
			my $stitching_part;
			if ( $stitching_service_index ) {
