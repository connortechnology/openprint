...
#$$specs{'RunTime'.$qty_index} = $price{RunTime};
	...
	$mprice = $rate * ($mprice + $ink + $$price{'Paper 1000 Price'} );
...
$$sig_price{Imposition}->display("ERROR got " . $$sig_price{'Comparison Cost'});
								} else {	
...
if ( 0 ) {
		...
		$string .= '<br/>'	. join(', ',
				...
				( defined $$specs{txtCropMarkSpace} ? () : 'no crop marks' ),
		);
