...
# We only specify GF spreads 1 at a time.
			...
			if ( ( $$specs{txtSpreadSize} == 4 and $$specs{'txtUnspecifiedSpreadQuantity'.$qty_index} > 250 ) or ( $$specs{'txtUnspecifiedSpreadQuantity'.$qty_index} > 500 ) ) {
...
# if $$Varnish{'run_price'};
	...
	if ( $Paper->type() ne 'Roll' ) {
		...
		$breakdown .= sprintf("\tPaper: \%sx\%s -> \%sx\%s * \%.6flbs/sq inch = \%.6f lbs per sheet (%d gsm, %sPT)<br/>", $Paper->start_width(), $Paper->start_height(), $Paper->width(), $Paper->height(), $Paper->wpsi(), $Paper->width() * $Paper->height()* $Paper->wpsi(), $Paper->gsm(), $Paper->calliper() );
...
sub get_project_price {
	...
	foreach my $Press ( ( $$specs{'chkOverridePress'.$qty_index} eq 'Y' and $$specs{'ddmPress'.$qty_index} ) ? openprint::Equipment::find('strid'=>$$specs{'ddmPress'.$qty_index} ) : ('', @$possible_presses) ) {
...
$openprint::log->debug("Max pages: $max_pages imps " . @impositions);
            $max_pages = ceil( $max_pages / 3 );
$openprint::log->debug("Max pages: $max_pages press".$Press->strid());
            foreach my $imp ( @impositions ) {
				my $Paper = $imp->Paper();
				...
				if ( $$specs{'chkOverrideSheetSize'.$qty_index} eq 'Y') {
					if ( ( $$Paper{'width'} != $$specs{"OverrideStockWidth$qty_index"}) and ( (! $$specs{"OverrideStockHeight$qty_index"} ) or $$Paper{'height'} != $$specs{"OverrideStockHeight$qty_index"} )) {
						next;
					} # end if
				} 
				if ( $$specs{'OverrideCutOff'.$qty_index} eq 'Y' ) {
					$openprint::log->warn("Want " . $$specs{'CutOff'.$qty_index} . ' got ' . $$Paper{'height'} );
					if ( $$Paper{'height'} != $$specs{'CutOff'.$qty_index} ) {
                ...
                if ( $imps{$str} ) {
                    for ( my $j = 0; $j < @{$imps{$str}}; $j += 1 ) {
                        ...
                        if ( ($$specs{'chkOverrideSheetSize'.$qty_index} eq 'Y') and ( $$Paper{'width'} == $$specs{"OverrideStockWidth$qty_index"}) and ( $$Paper{'height'} == $$specs{"OverrideStockHeight$qty_index"} ) ) {
                            next;
                        } elsif ( ( $$specs{'OverrideCutOff'.$qty_index} eq 'Y' ) and ( $I->Paper()->height() == $$specs{"CutOff$qty_index"} ) ) {
                            next;
                        } # end if
...
#$openprint::log->debug(sprintf('%sx%s->%sx%s $s %d $%s', $imp->Paper()->get('start_width','start_height','width','height','minimum_order','is_cut'), $SmallerPrice{'100lb'} ) );
                        ...
                        if (
                                ( $I->Paper()->area() >= $Paper->area() )
                                and
                                ( $I->Paper()->minimum_order() >= $Paper->minimum_order() )
                                and
                                ( (1*$BiggerPrice{'100lb'}) >= (1*$SmallerPrice{'100lb'}) )
                                and
                                ( ! ( ! $I->Paper()->is_cut() and $Paper->is_cut() ) )
                           ) {
#$openprint::log->debug('splice');
                            ...
                            $j -= 1;
                                ...
                                ( (1*$BiggerPrice{'100lb'}) <= (1*$SmallerPrice{'100lb'}) )
                                and
                                ( ( ! $I->Paper()->is_cut() ) or ( $Paper->is_cut() ) )
                                ) {
# Already have a much better sheet
							$add = 0;
...
$openprint::log->debug("Impositions for Press: " . $Press->strid() . ' after folding:' . @impositions) if $debug;
        ...
        if ( $$specs{'chkOverrideImposition'.$qty_index} eq 'Y' ) {
			...
			if ( ! $found ) {
				my @i;
				foreach my $I ( @impositions ) {
					next if $I->imposition() < $$specs{'txtImposition'.$qty_index};
...
$openprint::log->debug("QTY after filter: $qty_index on " . $P->strid() );
	...
	#$imp->display();
			...
			if ( $$specs{'PreviousStockType'} and ( $$Paper{'type'} ne $$specs{'PreviousStockType'} ) ) {
				$openprint::log->debug("Not consider imposition cuz it's not the previous stock type " . $Paper->type() ) if $debug;
				next;
			} # end if
			if ( $$specs{'chkOverrideGrainDirection'.$qty_index} eq 'Y' ) {
			...
			if ( $$specs{'chkOverrideSheetSize'.$qty_index} ) {
				if ( $$specs{'OverrideStockHeight'.$qty_index} ) {
					if ( $$Paper{'width'} != $$specs{'OverrideStockWidth'.$qty_index} or $$Paper{'height'} != $$specs{'OverrideStockHeight'.$qty_index} ) {
						$openprint::log->debug("Wrong stock want : ".$$specs{'OverrideStockWidth'.$qty_index}.'x'.$$specs{'OverrideStockHeight'.$qty_index}." but have " . $$Paper{'width'} . 'x'.$$Paper{'height'} );
						next;
					} # end if
				} else {
...
#$openprint::log->debug("First imp added " . @{$$price{'Impositions'}} );
...
#$new_specs{'no_stitching'} = 0; # unneccessary calculation
						...
						if ( int($$sig_price{'Comparison Cost'}) == $last_sig_price ) {
							...
							#$openprint::log->debug("Added $sigs impos " . @{$$price{'Impositions'}});
...
$PlateCounts{'Blank'.$$sig_price{'Plate Costs'}{'Plate ID'}} += $$sig_price{'Plate Costs'}{'Blank Plates'};
							...
							push @{$$price{'Impositions'}},$imp;
							#$openprint::log->debug("Added 1 impos " . @{$$price{'Impositions'}});
							$$specs{'txtUnspecifiedSpreadQuantity'.$qty_index} -= $imp->spreads();
...
#$openprint::log->warn("Doing full calc without Press Override" . $imp->spreads() );
							...
							if ( ! $$sig_price{'complete'} ) {
								...
								# There must always be at least 1 imposition in this.
								push @{$$price{'Impositions'}}, @{$$sig_price{'Impositions'}};
#$openprint::log->debug("FROM get_project_price: " . @{$$sig_price{'Impositions'}} . ' impositions. now at ' . @{$$price{'Impositions'}} );
...
# Now that impressiosn is set, we can round up the Sheet Count to take into account the number of sheets in a package
	...
	if ( $$Imposition{runstyle} eq 'Sheet Work' ) {
		if ( @$side_one_colours and @$side_two_colours ) {
			$_ = press_setup_cost( $openprint::log, $openprint::dbh, $openprint::variable, $Press, $$specs{'txtPlateChangeQuantity'.$qty_index}/2, $plate_setup{'Plate Runs'}, $side_one_colours, $$Paper{calliper}, $specs, $qty_index, $Project, $service_index, $Imposition );
			$press_setup += $_->{'Press Setup'};
			$price{'Plate Total'} += $_->{'Plate Total'};
			$_ = press_setup_cost( $openprint::log, $openprint::dbh, $openprint::variable, $Press, $$specs{'txtPlateChangeQuantity'.$qty_index}/2, $plate_setup{'Plate Runs'}, $side_two_colours, $$Paper{calliper}, $specs, $qty_index, $Project, $service_index, $Imposition );
			$press_setup += $_->{'Press Setup'};
			$price{'Plate Total'} += $_->{'Plate Total'};
...
#$openprint::log->debug("Area: $area Impressions: $impressions " . $Imposition->object_area() );
	...
	$price{'Comparison Cost'} += $price{'Ink Mix Charge'} + $price{'Press Wash Total'} + $price{'Film Cost'} + $price{'Ink Price'} + $varnish_price{'run_price'} + $varnish_price{'Material Total'};
#$openprint::log->debug("Comparison Cost: $price{'Comparison Cost'}");
...
# now work out the press run speed
	...
	my $speed_mod = $std_speed / $run_speed if $std_speed != $run_speed and $run_speed;
#$Press->specification('Press Additional Run Speed',$Imposition->paper()->calliper());
#$openprint::log->warn("Press ".$Press->strid()." Calliper:". $Imposition->paper()->calliper()." ($running_price) ($run_price{'units'}) STD: ($std_speed) RUN ($run_speed),  std/run: " . $speed_mod  )  if $debug or 1;
