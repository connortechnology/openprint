...
my %variables = (
		...
		'chkOverrideSignatureSpreadQuantity1' => ['save'], 'chkOverrideSignatureSpreadQuantity2' => ['save'], 'chkOverrideSignatureSpreadQuantity3' => ['save'],
...
sub get_unspecified_spreads {
	my ( $Project, $service_index, $printing_specs, $specs, $qty_index ) = @_;

	my @signature_indexes = $Project->signatures( $$specs{'txtSignatureType'} );

	my $specified_spreads = 0;
	foreach my $ssid ( @signature_indexes ) {
		next if $service_index and ( $ssid >= $service_index );
		my $sig_specs = openprint::service::get_specs_ref( $Project, $ssid );
		$specified_spreads += $$sig_specs{"txtSignatureSpreadQuantity$qty_index"};
	} # end foreach

	my $unspecified_spreads = 0;
	if ( $$specs{'txtSignatureType'} eq 'Interior Spreads' ) {
		$unspecified_spreads = $$printing_specs{'txtInteriorSpreadQuantity'} - $specified_spreads;
	} elsif ( $$specs{'txtSignatureType'} eq 'GateFolded Spreads' ) {
		$unspecified_spreads = $$printing_specs{'txtGateFoldedSpreadQuantity'} - $specified_spreads;
	} elsif ( $$specs{'txtSignatureType'} eq 'Cover Spreads' ) {
...
# Perfect bound requires more width on th cover to conver the calliiper	
				...
				foreach my $signature_service_index ( @signature_service_indices ) {
					...
					my $calliper3 = $$sig_specs{'txtSignatureSpreadQuantity3'} ? $$sig_specs{'txtSignatureSpreadQuantity3'} * $$sig_specs{'txtSpecificStockCalliper'} : $$sig_specs{'txtSpecificStockCalliper'};
					if ( $calliper1 ) {
						$finished_calliper += $calliper1 * $$sig_specs{'txtSpreadSize'}/2;
					} elsif ( $calliper2 ) {
						$finished_calliper += $calliper2 * $$sig_specs{'txtSpreadSize'}/2;
					} elsif ( $calliper3 ) {
						$finished_calliper += $calliper3 * $$sig_specs{'txtSpreadSize'}/2;
					} # end if
...
# If no spreadsize, then we are likely not a book, and the spread size is 2
	...
	if ( (! exists $$specs{'rdbColourBar'} ) or ( $$specs{'ScreenType'} eq 'FM' and $$specs{'rdbColourBar'} ne 'Y' ) ) {
		$variables{'rdbColourBar'} = [ sets::union( 'output', @{$variables{'rdbColourBar'}} ) ];
		$$specs{'rdbColourBar'} = 'Y';
	} else {
		$variables{'rdbColourBar'} = [ sets::exclude( ['output'], $variables{'rdbColourBar'} ) ];
	} # end if
	...
	if ( $$specs{'rdbSpecificStock'} eq 'Y' ) {
		...
		if ( ! $$specs{'CustomStockPrice'} ) {
			$$specs{'alert'} .= 'Please enter the stock price';
			return $$specs{'Status'} = 'uncalculated';
		} # end if
		...
		@$Paper{'cuttable','perfecting','calliper','doublesided','gsm','grade'} = ( 'Y','N',@$specs{'txtSpecificStockCalliper','CustomSheetDoubleSided','txtStockGSM','StockGrade'});
...
# Figure out how many spreads we need!
		if ( $$specs{'txtSignatureType'} ) {
			if ( $$specs{'txtSignatureType'} eq 'Interior Spreads' ) {
				$$specs{'totalSpreads'} = $$printing_specs{'txtInteriorSpreadQuantity'};
			} elsif ( $$specs{'txtSignatureType'} eq 'GateFolded Spreads' ) {
				$$specs{'totalSpreads'} = $$printing_specs{'txtGateFoldedSpreadQuantity'};
			} elsif ( $$specs{'txtSignatureType'} eq 'Cover Spreads' ) {
				$$specs{'totalSpreads'} = 1 * $$specs{'txtSpreadSize'};
			} else {
				$openprint::log->error("Unknown spread type $$specs{'txtSignatureType'}");
				return $$specs{'Status'} = 'uncalculated';
			} # end if

			$$specs{'txtUnspecifiedSpreadQuantity'.$qty_index} = get_unspecified_spreads( $Project, $service_index, $printing_specs, $specs, $qty_index );
			$$specs{'txtUnspecifiedSpreadQuantity'.$qty_index} = 0 if $$specs{'txtUnspecifiedSpreadQuantity'.$qty_index} < 0;
			if ( $$specs{'txtUnspecifiedSpreadQuantity'.$qty_index} and ($$specs{'txtSignatureType'} eq 'GateFolded Spreads' ) ) {
...
#$log->error( $$specs{'ddmPress'.$qty_index} . ' not in ' . join(',', @possible_presses ) );
			...
			if ( $$specs{'txtSignatureType'} ) {
				if ( $$specs{'chkOverrideSignatureSpreadQuantity'.$qty_index} eq 'Y' ) {
					$project{'SpreadLayout'} = $$specs{'txtSignatureSpreadQuantity'.$qty_index};
...
$openprint::log->debug("No spread layout for you!");
			...
			if ( (exists $project{'SpreadLayout'}) and ! $project{'SpreadLayout'} ) {
				...
				$$specs{"txtSignatureSpreadQuantity$qty_index"} = 0;
				$$specs{'alert'} .= "No more spreads need to be specified for quantity $qty_index.";
				next;
			} # end if
...
$openprint::log->debug("Not in Web Estimating group");
				next;
			} # end if
...
# not all of the presses have a gutter spec so we will continue to use Grip for Width and Height
			...
			if ( $project{'Add Colour Bar'} eq 'Y' ) {
				if ( 4 == sets::union( 'Cyan','Magenta','Yellow','Black', @side_one_colours, @side_two_colours ) ) {
				...
				foreach my $Paper ( @Papers ) {
					if ( $Paper->type() eq 'Roll' ) {
						if ( $$specs{'chkOverrideSheetSize'.$qty_index} eq 'Y' ) {
							...
							my $width = $$specs{'OverrideStockWidth'.$qty_index } ? $$specs{'OverrideStockWidth'.$qty_index } : $$specs{'ddmStockSheetSize'.$qty_index};
							next if $Paper->start_width() and $Paper->start_width() != $width;
							if ( ! $Paper->start_width() ) {
								$Paper->start_width( $width );
								$Paper->width( $width );
							} # end if
						} # end if
...
# Keep cutting whiel the sheet still fits on th press.  I originally thought that we shouldn't do this, because why would youw ant to run a half sheet if a full sheet fits?  The answer: small jobs, that due to overs requires the same sheets whether you run full or half.  So by running half, you need half the # of real sheets.
						while (
							...
							if ( $$specs{'chkOverrideSheetSize'.$qty_index} eq 'Y' ) {
								my ( $width, $height ) = $$specs{'OverrideStockWidth'.$qty_index} ? @$specs{'OverrideStockWidth'.$qty_index,'OverrideStockHeight'.$qty_index} : split('x', $$specs{'ddmStockSheetSize'.$qty_index});
								$openprint::log->debug("Override Sheet Size want:$width x $height Looking at " . $P->width() . 'x'.$P->height()) if $debug;
								last if ! $height;
...
my %best_price = %{$b_price};
		...
		my $Varnish = $best_price{'Varnish'};
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("Colour Bar \%s \%s<br/>", $Imposition->colour_bar_size(), $Imposition->colour_bar_orientation() );
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("Setups:\n\tPress Setup:\t\t\t\$%.2f<br/>", $best_price{'Press Setup'} );
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tImposition Charge:\t\$%1\$.2f + \$%2\$.2f*\%4\$d=\$%3\$.2f<br/>", @best_price{'Imposition MakeReady','Imposition Price','Imposition Total'}, $Imposition->imposition() );
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tRunstyle Charge:\t\$%.2f<br/>", $best_price{'Runstyle Charge'} );
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tWork & Turn Dry Cost:\t\$%.2f<br/>", $best_price{'WorkTurn Dry Charge'} ) if $best_price{'WorkTurn Dry Charge'};
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tAqueous Setup:\t\$%.2f<br/>", $$Aqueous{'Setup'}) if $$Aqueous{'Setup'};
		...
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tInline Varnish Charge: \$%.4f\%s = %.2f<br/>", @$Varnish{'run_price','Run Units','Run Total'} );
# if $$Varnish{'run_price'};
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tAqueous Run Charge: %.2f%s = \$%.2f<br/>", @$Aqueous{'Run Cost','Units','Total'} );
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tMinimum Run Charge: \$%.2f<br/>", $best_price{'Minimum Run Charge'} );
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tRun Charge Total:\t\$%.2f<br/>", $best_price{'Run Total'} );
		$$specs{'hdnBreakdown'.$qty_index} .= "Material Charges:<br/>";
		my $plate_costs = $best_price{'Plate Costs'};
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf( "\tPlates: \%d plates * \$%.2f per plate = \$%.2f<br/>", @best_price{'txtPlateQuantity','Plate Cost','Plate Price'});
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf( "\tBlank Plates: \%d plates * \$%.2f per plate = \$%.2f\n", @$plate_costs{'Blank Plates','Blank Price'}, $$plate_costs{'Blank Price'} * $$plate_costs{'Blank Plates'});
		...
		if ( $Paper->type() ne 'Roll' ) {
			$$specs{'hdnBreakdown'.$qty_index} .= "\tPaper: $best_price{'Gross Sheet Count'} sheets * \$ $best_price{'Sheet Price'} ";
			...
			$best_price{'Paper 1000 Price'} = ($best_price{'Sheet Price'}*1000/$Imposition->imposition());
		} elsif ( $Paper->width() ) {
			$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tPaper: \%sx\%s * \%.6flbs/sq inch = \%.6f lbs per sheet (%d gsm)<br/>", $Paper->width(), $Paper->height(), $Paper->wpsi(), $Paper->width() * $Paper->height()* $Paper->wpsi(), $Paper->gsm() );
			$best_price{'Paper 1000 Price'} = (($Paper->mweight()/$Imposition->imposition())/100)*$best_price{'100lb'};

			$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tPaper: %.0f lbs * \$%.2f/100lb = \$%.2f<br/>", @best_price{'Stock Weight','100lb','Paper Price'});
		} # end if
		if ( $$specs{'rdbSuppliedStock'} eq 'Y' ) {
			$$specs{'hdnBreakdown'.$qty_index} .= "\tPaper Price not included in total<br/>";
		} # end if
		$$specs{'hdnBreakdown'.$qty_index} .= $best_price{'Ink breakdown'};
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tInk Total: \$%.2f<br/>", $best_price{'Ink Price'} );
		$$specs{'hdnBreakdown'.$qty_index} .= sprintf("\tVarnish: \$%.4f\%s = \$%.2f<br/>", @$Varnish{'Material Price','Material Units','Material Total'} );
		$$specs{'hdnBreakdown'.$qty_index} .= "Total: $best_price{'Total Cost'}<br/>";
		$$specs{'hdnBreakdown'.$qty_index} .= $best_price{'Folding Breakdown'};
		$$specs{'hdnBreakdown'.$qty_index} .= $best_price{'Cutting Breakdown'};
		$$specs{'hdnBreakdown'.$qty_index} .= $best_price{'Scoring Breakdown'};
...
#$$specs{'RunTime'.$qty_index} = $best_price{'RunTime'};
		...
		$$specs{'MPrice'.$qty_index} = sprintf('%.2f', ((($best_price{'Impression Price'} + $$Varnish{'Run Total'} + $$Varnish{'Material Total'} + $$Aqueous{'Total'} + $best_price{'Ink Price'} + $best_price{'PMS Run Total'})/ $qty)*1000 ) + $best_price{'Paper 1000 Price'} );

		if ( $$specs{'txtSignatureType'} ) {
			if ( $$specs{'chkOverrideSignatureSpreadQuantity'.$qty_index} eq 'Y' ) {
				$variables{'txtSignatureSpreadQuantity'.$qty_index} = [ sets::exclude( ['output'], $variables{'txtSignatureSpreadQuantity'.$qty_index} ) ];

				$$specs{'PageQuantity'.$qty_index} = $$specs{'txtSignatureSpreadQuantity'.$qty_index} * $$specs{'txtSpreadSize'};
			} elsif ( $$specs{'chkOverridePageQuantity'.$qty_index} eq 'Y' ) {
				$variables{'txtSignatureSpreadQuantity'.$qty_index} = [ sets::union( 'output', @{$variables{'txtSignatureSpreadQuantity'.$qty_index}} ) ];
...
my $b = $$specs{'hdnBreakdown'.$qty_index};
$b =~ s/<br\/>/\n/g;
$openprint::log->debug("Breakdown: $b");
...
#$imp->display();
			...
			next if ( ( $$specs{'chkOverridePageQuantity'.$qty_index} eq 'Y' ) and ( $imp->pages() != $$specs{'PageQuantity'.$qty_index} ) );
...
# We assume that calc_price does not alter the specs hash.  
						...
						$$sig_price{'Comparison Cost'} -= $$sig_price{'Stitching Cost'};
...
#$openprint::log->debug("Unable to calculated");
					...
					if ( $$sig_price{'Comparison Cost'} < 0 ) {
						$$price{'complete'} = 0;
						last;
					} # end if	

					if ( $$sig_price{'Comparison Cost'} == $$price{'Comparison Cost'} ) {
						$$sig_price{'Comparison Cost'} *= $$specs{'txtUnspecifiedSpreadQuantity'.$qty_index}/$imp->spreads();
						$$specs{'txtUnspecifiedSpreadQuantity'.$qty_index} = $imp->spreads();
					} # end if
					...
					$$price{'AdditionalSignature Breakdown'} .= 'Setup: ' . $$sig_price{'Setup Total'} . '<br/>';
					$$price{'AdditionalSignature Breakdown'} .= 'Run: ' . $$sig_price{'Run Total'} . '<br/>';
					...
					$$price{'AdditionalSignature Breakdown'} .= 'Additional Signature: ' . sprintf('%.2f', $$sig_price{'Comparison Cost'} ) . '<br/>';
					$$specs{'txtUnspecifiedSpreadQuantity'.$qty_index} -= $imp->spreads();
...
sub check_price {
	...
	#if ( $$specs{'txtUnspecifiedSpreadQuantity'.$qty_index} ) {
...
sub calc_price {
	...
	$$specs{'SpreadCols'.$qty_index} = $Imposition->spread_columns();
	$$specs{'hdnImageOrientation'.$qty_index} = $Imposition->image_orientation();
	$$specs{"txtSignatureSpreadQuantity$qty_index"} = $Imposition->spreads();
	$$specs{'ddmPress'.$qty_index} = $Press->strid();
	$$specs{'txtStockGSM'} = $Paper->gsm();
	$$specs{'ddmRunStyle'.$qty_index} = $Imposition->runstyle();
	...
	$price{'txtSignatureSpreadQuantity'} = $Imposition->spreads() ? $Imposition->spreads() : 1;
...
# Now that impressiosn is set, we can round up the Sheet Count to take into account the number of sheets in a package
	...
	#Initially we calculate based on colours, but really we need to calculate based on plates, which we will do once we figure out how many plates we need.
	my $min_overs = $Press->specification( 'Press Run Overs Minimum', $plate_setup{'Plate Count'} );
	my $setup_rate = $Press->specification( 'Press Run Overs Rate', $plate_setup{'Plate Count'} );
	my $setup_overs = $setup_rate * ( $plate_setup{'Plate Count'} );
	$setup_overs += $Press->specification( 'FM Screening Additional Overs', undef ) if $$specs{'ScreenType'} eq 'FM';
	$setup_overs = $min_overs if $setup_overs < $min_overs;
...
$openprint::log->debug("Sheets Per Package: ($sheets_per_package)");
	...
	return if check_price( $price_to_beat, \%price, $specs, $qty_index, $Imposition, 'Paper' );
...
#my $time = gettimeofday();
		%folding_results = openprint::Estimating::Folding::signature_calc( $openprint::log, $openprint::dbh, $openprint::variable, $Project, $service_index, $specs, $$Project{'FoldingSpecs'}, $qty_index, $Paper, $Imposition );
...
$openprint::log->warn("Got no runspeed.");
		...
		return if check_price( $price_to_beat, \%price, $specs, $qty_index, $Imposition, 'Folding' );
	} # end if
...
#foreach my $key ( keys %aqueous ) {
#$openprint::log->debug("AQUEous $key => $aqueous{$key}");
#}
	my %varnish_price;
	if ( sets::isin( $Imposition->runstyle(), ['Work & Turn','Work & Tumble'] ) ) {
...
#$openprint::log->debug("Comparison Cost: $price{'Comparison Cost'}");
	return if check_price( $price_to_beat, \%price, $specs, $qty_index, $Imposition, 'Run Charges' );
	...
	return if check_price( $price_to_beat, \%price, $specs, $qty_index, $Imposition, 'Run Cost' );
...
#$openprint::log->debug("Comparison Cost: $price{'Comparison Cost'}");
	return if check_price( $price_to_beat, \%price, $specs, $qty_index, $Imposition, 'Totals' );
	...
	if ( $$services{'SaddleStitching'} and $$specs{'txtSignatureType'} ne 'Cover Spreads') {
		...
		return if check_price( $price_to_beat, \%price, $specs, $qty_index, $Imposition, 'Saddle Stitching' );
	} # end if
...
#$openprint::log->debug("Elapsed stock cutting time:" . ( sprintf('%.4f', tv_interval( [$time])*1000) ) .' usecs' );
			return if check_price( $price_to_beat, \%price, $specs, $qty_index, $Imposition, 'Stock Cutting' );
		} # end if
...
#$log->debug(" ** Current Project Types is: $project_type ** ");
	...
	foreach my $Press ( @presses ) {
		...
		if ( $Paper->calliper() > $Press->specification('Maximum Calliper') ) {
...
#$log->warn(" ** FINAL  RUNNING PRICE $running_price **") if $debug or 1;
	...
	} elsif ( lc $run_price{'units'}, 'per hour' ) {
		if ( $run_speed ) {
