...
# Don't cut sheets into rolls
					...
					if ( ! (
						...
						$openprint::log->debug("No good: $qty_index " . $P->id_string() . ' '. ($P->start_width() % $$specs{'OverrideStockWidth'.$qty_index}) . 'x' . ($P->start_height() % $$specs{'OverrideStockHeight'.$qty_index} ) );
...
# Make sure gsm has calculated
	...
	foreach my $P ( @Papers ) {
		$openprint::log->debug("Base Paper: " . $P->id_string() . ' Minimum: ' . $P->minimum_order() ) if ( $debug or 1 );
	...
	foreach my $Press ( @$Presses ) {
		...
		if ( $$specs{'OverridePrintingType'.$qty_index} eq 'Y' ) {
			if ( $Press->specification('Printing Type') ne $$specs{'PrintingType'.$qty_index} ) {
				...
				$openprint::log->warn("QTY $qty_index Press $$Press{strid} Printing Type (" . $Press->specification('Printing Type') .") IS the overriden type " . $$specs{'PrintingType'.$qty_index} ) if $debug;
			} # end if
...
#$openprint::log->debug('blah'.$Paper->id_string());
...
#$openprint::log->debug("Getting impos for $$Press{strid} " . $P->id_string() );
					my @i = openprint::imposition::get_imposition( $project, $do_work_turn, $do_perfecting, $$specs{'Versions'}, $P,
							undef, 
							undef, 
							#$$specs{'rdbGrainDirection'.$qty_index},
							$Press );
#$openprint::log->debug("Back from Getting impos for $$Press{strid} " . $P->id_string() );
#foreach my $i ( @i ) {
	#$i->display();
#}
			...
			if ( $debug or 0 ) {
				$openprint::log->debug("Sorting from paper " . $Paper->id_string() . ' on ' . $Press->strid() );	
				foreach my $i ( @imps ) {
					$i->display();
				}
			}
			if ( $debug or 0 ) {
				foreach my $P ( @$Papers ) {
					$openprint::log->debug("Paper: " . $P->id_string() );
				} # end foreach
			} # end if
			foreach my $imp ( @imps ) {
...
#
		...
		my $ImpositionCharge = $best_price{'Imposition Price'};

		my $PageCharge = $$ImpositionCharge{'Page Charge'};
		$$specs{'PageCharge'.$qty_index} = $$PageCharge{'Total'};
		my $SteppingCharge = $$ImpositionCharge{'Stepping Charge'};
...
$openprint::log->debug("qti: $qty_index Spread: $SpreadLayout");
	} # end if
...
#$log->debug("Stock qty: ($qty/$$imp{imposition})=$stock_qty lbs paper_counts: " . $$PaperCounts{$Paper->id_string()});
			#} else {
				#$stock_qty
			#} # end if
			#$stock_qty += $$PaperCounts{$Paper->id_string()};
...
$openprint::log->debug("Paper debug: " . $Paper->id_string() );
...
$openprint::log->debug("Using cached folding");
			...
			%folding_results = openprint::Estimating::Folding::signature_calc( $Project, $service_index, $specs, $$project{'FoldingSpecs'}, $qty_index, $Imposition, @$project{'UVCoatingSpecs','AqueousSpecs','StitchingSpecs'}, $other_impositions );
...
#$openprint::log->debug("Got price for $colour");
		...
		if ( lc $ink_price{'units'} eq 'per cartridge' ) {
			...
			$price{'Ink breakdown'} .= sprintf(' mileage: %d, %.2f * $%s%s=$%.2f', $$Coverage{value}, $qty, @ink_price{'Price','units'},$ink_price{'Price'} * $qty);
...
# Digital only support Process, no PMS, etc...
		...
		if ( my $stocknames = $Press->specification('StockNames') ) {
			...
			if ( @allowed and ! sets::isin( $Paper->name(), \@allowed ) ) {
				$results{$press_id} = 'Not suitable for this stock.';
				next;
			} # end if
			if ( @disallowed and sets::isin( $Paper->name(), \@disallowed ) ) {
                $results{$press_id} = 'Not suitable for this stock.';
                next;
            } # end if
		} # end if
