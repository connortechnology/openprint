...
#$openprint::log->debug("Adding " . $Fold->pages() . 'x'.$Fold->quantity() );
				...
				$$specs{"txtPockets$qty_index"} += $Fold->page_quantity();
			} # end foreach Fold
		}
		...
		if ( $imposition > 1 ) {
			...
			if ($$I{imposition} % 2 ) {
				...
				$results{Breakdown} .= "Setting imposition to 1 due to Horizontal and odd cols on form $form<br/>";
				$imposition = 1;
...
# Convert to cost per thousand
	...
	$price{Price} = Math::Round::nearest(0.01,$price{'MakeReady'} + $price{'Service'} + $price{'Insert'});
