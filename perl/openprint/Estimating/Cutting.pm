...
my @variables = (
        'ddmEquipment1', 'chkOverrideEquipment1',
        'ddmEquipment2', 'chkOverrideEquipment2',
        'ddmEquipment3', 'chkOverrideEquipment3',
        'ddmStockCutEquipment1', 'chkOverrideStockCutEquipment1',
        'ddmStockCutEquipment2', 'chkOverrideStockCutEquipment2',
        'ddmStockCutEquipment3', 'chkOverrideStockCutEquipment3',
...
sub signature_calc {
	...
	$$sig_specs{'hdnBreakdown'.$qty_index} = '';
	$Paper = openprint::Paper::load_from_signature( $Project, $sig_specs, $qty_index ) if ! $Paper;
...
# Grab the Calliper
	...
	my $ProjectTypeID = $ProjectType->strid();

# Done Stock Cuts, can now do regular cuts
	my $bestPrice = undef;
	my $bestEquipment;
	foreach my $Equipment ( @my_equipment ) {
		...
		if ( my $reason = $Equipment->fits( $sheet_width, $sheet_height, $$specs{'txtStockCalliper'} ) ) {
...
# For the non-book case, this devolves into the printing service
		foreach my $signature_service_index ( $Project->signatures() ) {
			...
			if ( ( $$sig_specs{'StockType'.$qty_index} ne 'Roll' ) and ( $$sig_specs{"hdnSuppliedStockWidth$qty_index"} != $$sig_specs{'StockWidth'.$qty_index} or $$sig_specs{"hdnSuppliedStockHeight$qty_index"} != $$sig_specs{'StockHeight'.$qty_index} ) ) {
				my %results = signature_calc_stock_cutting( $log, $dbh, $variable, $Project, $signature_service_index, $sig_specs, $specs, $qty_index );
			...
			$$specs{"txtRegularCutPrice-$signature_index-$qty_index"} = $results{'Price'};
			$price += $results{'Price'};
			$mprice += $results{'MPrice'};
...
sub display {
	...
	@{$$variable{'StockCutEquipmentArray'}} = map { $_->id(), $_->name() } sets::exclude( \@stitchers, \@possible_equipment );
