...
sub calc {
	...
	foreach my $qty_index ( $Project->quantity_indexes() ) {
		...
		foreach my $Equipment ( @Equipment ) {
			...
			my %MakeReady = openprint::service::get_price_object(undef, undef, undef, 'DTapingMakeReady', undef, $Equipment );
			if ( ! %MakeReady ) {
			...
			foreach my $ss_id ( $Project->signatures() ) {
				...
				foreach my $side ( 'top','bottom','left','right' ) {
					...
					if ( $runs ) {
						my %ServicePrice = openprint::service::get_price_object(undef,undef,undef,'DTaping',$dtapes_per_run ? $dtapes_per_run : undef, $Equipment ); 
						if ( ! %ServicePrice ) {
					...
					if ( $last_run ) {
						my %LastServicePrice;
						%LastServicePrice = openprint::service::get_price_object(undef,undef,undef,'DTaping',$last_run, $Equipment );
						if ( ! %LastServicePrice ) {
