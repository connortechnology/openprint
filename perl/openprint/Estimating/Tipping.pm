...
sub calc {
	...
	my @Equipment = openprint::Equipment->find('Specifications'=>{'Tipping Capable'=>'Y'},'useinestimating'=>1);
	push @Equipment, openprint::Equipment->find('Specifications'=>{'Tipping Capable'=>'When PerfectBound'},'useinestimating'=>1) if $$services{'PerfectBind'};
	push @Equipment, openprint::Equipment->find('Specifications'=>{'Tipping Capable'=>'When Stitching'},'useinestimating'=>1) if $$services{'SaddleStitching'} or $$services{'LoopStitching'};
	if ( ! @Equipment ) {
...
sub display {
	...
	my $services = $Project->services();
	my @possible_equipment = openprint::Equipment->find( 'Specifications' => {'Tipping Capable'=>'Y'}, 'useinestimating'=>1,'order'=>'lower(strName)');
	push @possible_equipment, openprint::Equipment->find( 'Specifications' => {'Tipping Capable'=>'When Stitching'}, 'useinestimating'=>1,'order'=>'lower(strName)') if $$services{'SaddleStitching'} or $$services{'LoopStitching'};
	push @possible_equipment, openprint::Equipment->find( 'Specifications' => {'Tipping Capable'=>'When PerfectBound'}, 'useinestimating'=>1,'order'=>'lower(strName)') if $$services{'PerfectBound'};
