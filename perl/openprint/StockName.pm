...
sub load {
	...
	if ( (! $data) and $$self{'id'} ) {
		$data = $openprint::dbh->selectrow_hashref( 'SELECT * FROM PaperNames WHERE id=?', {}, $$self{'id'} );
...
sub delete {
	my $self = shift;
    sql::execute( undef, undef, q{DELETE FROM PaperNames WHERE id=?}, $$self{'id'} );
} # end sub delete
...
sub save {
	...
	if ( ! $$self{'id'} ) {
		...
		sql::insert( undef, undef, 'PaperNames', $self );
	} else {
		sql::update( undef, undef, 'PaperNames', ['id=?', $$self{'id'}], $self );
	} # end if
} # end sub save

sub copy {
	my $self = shift;
	my $new = new openprint::StockName();
